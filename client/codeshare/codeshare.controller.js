...
.controller('CodeShareController', ['$scope', '$http', '$state', '$window', 'socket', 'Account', '$log', 'profiledata', function($scope, $http, $state, $window, socket, Account, $log, profiledata) {
  ...
  var coderoom = $scope.joinedRoom;
    ...
    $scope.initSharing = function(user) {
      $scope.id++;
      socket.emit("startLiveEditing", {
        toName: user,
        fromName: Account.getUserDisplayName()
      });
        ...
        $scope.modeChanged = function() {
          ...
          onChange: function(_ace) {
            ...
            if ($scope.textInEditor !== sessionDoc.getValue()) {
                ...
                socket.emit($scope.title, {
                  title: $scope.title,
                  textFromDoc: $scope.textInEditor
                });
              }
    ...
    if (Account.getCheckIfLoggedOut() === 'false') {
      $http.post('/retrievingDocumentsForUser', {
    ...
    $scope.add = function() {
      ...
      $http.post('/savingDocumentsToDatabase', {
        id: total,
        title: ($scope.title + $scope.fileTypes[$scope.mode]),
        mode: $scope.mode,
        displayName: Account.getUserDisplayName(),
        code: $scope.aceModel
      });
    ...
    $scope.delete = function(id) {
      ...
      $http.post('/deleteDocumentsForUser', {
        displayName: Account.getUserDisplayName(),
        title: item.title,
        id: item.id
